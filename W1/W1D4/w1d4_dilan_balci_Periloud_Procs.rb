...
def filter_out(arr,&prc)
    ...
    arr.each do |el|
        new_arr << el if !prc.call(el)   #=> false
    end
...
def every?(arr, &prc)
    ...
    # (arr.all? { |ele| prc.call(ele)})? true : false
...
def xnor_select(arr,prc1,prc2)
    ...
    arr.each do |el|
        if prc1.call(el) == true and prc2.call(el) == true
            new_arr << el
        elsif prc1.call(el) == false and prc2.call(el) == false
            new_arr << el
        end
    end
...
def multi_map(arr, number=1, &prc)
...
def chain_map(num, arr)
    ...
    result = arr.inject(num) {|acc, prc| prc.call(acc)}
...
# sentence is changed by a value proc if the original word returns true when passed into the key proc. 
...
def procipher(str, changer)
    ...
    str.split(" ").each do |word|
        ...
        final = word         
        changer.each do |keyprc , valprc|
            if keyprc.call(word) 
                final =  valprc.call(final)
            end
        end
        words << final
    end
...
def picky_procipher(str, changer)
    ...
    str.split(" ").each do |word|
        ...
        final = word         
        changer.each do |keyprc , valprc|
            if keyprc.call(word) and word == final
                final =  valprc.call(final)
            end
        end
        words << final
